...
import com.teleclub.rajatalk.adapter.TabPagerAdapter;
...
import com.teleclub.rajatalk.network.result.BalanceResult;
import com.teleclub.rajatalk.network.result.CanTopupResult;
import com.teleclub.rajatalk.network.result.GeneralResult;
import com.teleclub.rajatalk.network.result.LoginResult;
import com.teleclub.rajatalk.network.result.TokenResult;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void saveToken() {
        final SharedPreferences prefs = getSharedPreferences(Constants.RAJATALK_PREF, Context.MODE_PRIVATE);
        ...
        RetrofitBuilder.createRetrofitService(RajaTalkAPI.class).saveToken(AppData.account.getPhone(), "android", Util.getUniqueIMEI(getApplicationContext()), pushToken)
...
//                logout();
    ...
    private void login() {
        ...
        showIndicator(true);
        RetrofitBuilder.createRetrofitService(RajaTalkAPI.class).login(username, password, Util.getUniqueIMEI(this),
...
//        saveContacts();
    ...
    private void getBalance() {
        RetrofitBuilder.createRetrofitService(RajaTalkAPI.class).getBalance(AppData.token, Util.getUniqueIMEI(this), "android")
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void checkTopupAvailable() {
        RetrofitBuilder.createRetrofitService(RajaTalkAPI.class).getCanTopup(AppData.token, Util.getUniqueIMEI(this),
                Util.getSimCardNumber(this), "android")
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public void generateRootDirectory() {
        ...
        if (permission == PermissionChecker.PERMISSION_GRANTED) {
            File f = new File(Environment.getExternalStorageDirectory() + "/RajaTalk");
            if (!f.exists()) {
                f.mkdir();
            }
        } else {
    ...
    public void saveContacts() {
        ...
        if (array.length() > 0) {
            RetrofitBuilder.createRetrofitService(RajaTalkAPI.class).setContacts(AppData.token, array)
                    .subscribeOn(Schedulers.newThread())
                    .observeOn(AndroidSchedulers.mainThread())
